...
export default function SettingsClient({
  ...
  const handleSaveExisting = async (updatedMethod: PaymentMethod) => {
  ...
  const handleCancelNew = (id: string) => {
    setNewMethods((prev) => prev.filter((m) => m.id !== id))
  }
      ...
      <div className="flex flex-col gap-4">
        ...
        {newMethods.map((method) => (
          <PaymentMethodCard
            ...
            onDelete={async () => {}} // Not possible since it hasn't been saved yet
            onCancelNew={() => handleCancelNew(method.id)}
          />
        ))}
